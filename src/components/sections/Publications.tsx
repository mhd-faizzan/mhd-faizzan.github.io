...
import { groupAndPaginate } from "@/lib/utils";
...
export default function Publications() {
  ...
  const formatAuthors = (authors: string | string[]): JSX.Element => {
    const list = typeof authors === "string" ? authors.split(",").map((a) => a.trim()) : authors;
    return (
      <>
  ...
  const { visibleGroups, sortedKeys } = groupAndPaginate(publications, "year", visibleCount);
            ...
            <div key={year} className="space-y-2">
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm text-gray-600 bg-gray-100 px-3 py-1 rounded-full">
                  {visibleGroups[year]?.length} publication{visibleGroups[year]?.length > 1 ? "s" : ""}
                </span>
              </div>
                  ...
                  >
                    <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                      <div className="flex-1">
                        <h3 className="text-base font-bold text-gray-900 text-justify">{item.title}</h3>
                        <p className="text-gray-600 text-sm text-justify">{formatAuthors(item.authors)}</p>
                        {item.doi && (
